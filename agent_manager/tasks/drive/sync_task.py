"""Daily periodic synchronization task for Google Drive files."""
...
def daily_drive_sync(self) -> dict:
    """Kick off incremental sync for every active Google Drive agent."""
    db = SessionLocal()
    try:
        contexts = ThirdPartyContextRepository(db).get_all_by_integration_and_status(
            integration_name="google_drive", status="complete"
        )
        if not contexts:
            logger.info("Daily sync: no active Google Drive agents.")
            return {"scheduled": 0}
        ...
        for ctx in contexts:
            ...
            logger.info("Daily drive sync queued: agent=%s task=%s", ctx.agent_id, task.id)
