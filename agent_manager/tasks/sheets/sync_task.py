"""Daily periodic synchronization tasks for Google Sheets."""
...
def daily_sheets_sync(self) -> dict:
    """Kick off incremental sync for every active Google Sheets agent."""
    db = SessionLocal()
    try:
        contexts = ThirdPartyContextRepository(db).get_all_by_integration_and_status(
            integration_name="google_sheets", status="complete"
        )
        if not contexts:
            logger.info("Daily sync: no active Google Sheets agents.")
            return {"scheduled": 0}
        ...
        for ctx in contexts:
            ...
            logger.info("Daily sheets sync queued: agent=%s task=%s", ctx.agent_id, task.id)
