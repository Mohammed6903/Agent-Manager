"""Google Docs context provider implementation."""
...
class DocsContextProvider(IntegrationContextProvider):
    ...
    def verify_credentials(self, db: Session, agent_id: str) -> bool:
        from agent_manager.integrations.google.docs.service import get_service
    ...
    def snapshot(self, agent_id: str, hours: int = 168) -> list[dict]:
        from agent_manager.integrations.google.docs.search_service import snapshot
    ...
    def snapshot_label(self) -> str:
        return "Recently modified documents (last 7 days):"

    def semantic_label(self) -> str:
        return "Relevant documents for your query:"
    ...
    def expired_sync_cursor_http_code(self) -> int:
        return 403  # Drive returns 403 when page token is too old

    def build_api_service(self, credentials: Any) -> Any:
        # We use Drive API for discovery + export, not the Docs API
    ...
    def full_sync(
        ...
        from agent_manager.tasks.docs.ingest_task import _full_sync
    ...
    def incremental_sync(
        ...
        from agent_manager.tasks.docs.ingest_task import _incremental_sync
    ...
    def list_s3_item_ids(self, agent_id: str) -> list[str]:
        ...
        return s3_service.list_item_ids(agent_id, "docs")

    def load_s3_batch(self, agent_id: str, item_ids: list[str]) -> list[dict]:
        from agent_manager.services import s3_service
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {
                executor.submit(s3_service.load_raw, agent_id, "docs", did): did
                for did in item_ids
            }
            ...
            for future in as_completed(futures):
                ...
                if raw:
                    results[did] = raw
        return [results[did] for did in item_ids if did in results]

    def pipeline_batch(
        self, items: list[dict], agent_id: str, account_email: str,
    ) -> None:
        from agent_manager.integrations.google.docs.pipeline_service import process_docs_batch
        process_docs_batch(items, agent_id, account_email)

    def pipeline_single(
        self, item: dict, agent_id: str, account_email: str,
    ) -> None:
        from agent_manager.integrations.google.docs.pipeline_service import process_doc
        process_doc(item, agent_id, account_email)

    def delete_s3_data(self, agent_id: str, task_id: str, update_progress: Any) -> int:
        from agent_manager.services import s3_service
        return s3_service.delete_namespace(agent_id, "docs")
