...
class GoogleSlidesApiRequest(BaseModel):
    agent_id: str = Field(..., description="Agent with Google Slides integration")
...
@router.post("/request", tags=["Google Slides"])
def api_request(req: GoogleSlidesApiRequest, db: Session = Depends(get_db)):
    """Execute a Google Slides API request via Google SDK."""
    try:
