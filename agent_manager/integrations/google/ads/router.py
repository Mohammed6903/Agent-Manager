...
class GoogleAdsApiRequest(BaseModel):
    agent_id: str = Field(..., description="Agent with Google Ads integration")
...
@router.post("/request", tags=["Google Ads"])
def api_request(req: GoogleAdsApiRequest, db: Session = Depends(get_db)):
    """Execute a Google Ads API request via Google SDK."""
    try:
