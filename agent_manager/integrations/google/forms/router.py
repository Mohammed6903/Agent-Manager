...
class GoogleFormsApiRequest(BaseModel):
    agent_id: str = Field(..., description="Agent with Google Forms integration")
...
@router.post("/request", tags=["Google Forms"])
def api_request(req: GoogleFormsApiRequest, db: Session = Depends(get_db)):
    """Execute a Google Forms API request via Google SDK."""
    try:
