...
class GoogleSearchConsoleApiRequest(BaseModel):
    agent_id: str = Field(..., description="Agent with Google Search Console integration")
...
@router.post("/request", tags=["Google Search Console"])
def api_request(req: GoogleSearchConsoleApiRequest, db: Session = Depends(get_db)):
    """Execute a Google Search Console API request via Google SDK."""
    try:
