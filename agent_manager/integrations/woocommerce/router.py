...
from . import service
from .schemas import WooCommerceApiRequest

router = APIRouter()


@router.post("/request", tags=["WooCommerce"])
async def api_request(body: WooCommerceApiRequest, db: Session = Depends(get_db)):
    try:
